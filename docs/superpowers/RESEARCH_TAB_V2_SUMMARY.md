...
6. **Claim of "latent bug in old `renderLiveCard`" was imprecise.** The narrative in earlier notes said the callback wrote to targets that `card.innerHTML = ...` had already overwritten. That's semantically impossible — `.then` callbacks run on the microtask queue, strictly after synchronous code finishes, so by the time the fetch resolves the new stats div is already in place. The only real failure mode is **orphaned card references on re-render** (old card detached, fetch callback writes into a garbage-collected subtree), and `researchLoadLiveCards()` is currently called exactly once per session (gated by `researchState.loaded`), so that race does not fire in practice today. v2's rewrite in `9e7e4ef` is a cleanup, not a bug fix. If a future change adds a refresh path, protect it with an AbortController or a render-generation token before shipping.
