...
class RobustnessSuiteResult(BaseModel):
    """Bundle of all 5 robustness outputs + the aggregate verdict."""
...
def run_robustness_suite(
    ...
    """
    Run all four robustness tests and aggregate into a verdict.

    Args:
        skip: list of test names to skip — one or more of
              ['monte_carlo', 'param_landscape', 'entry_delay', 'loso'].
    ...
    if "monte_carlo" not in skip:
        mc = run_monte_carlo(backtest_result, n_simulations=mc_n_simulations)
    ...
    verdict = compute_verdict(
        ...
        entry_delay=ed, loso=lo, wf=wf_result,
    )
